/*
Firestore migration script: migrate documents from legacy collection -> `entries`.
...
Dry-run mode (default): prints planned changes without writing
  node scripts/migrate-legacy-to-entries.js --dry
Apply mode: performs the migration
  node scripts/migrate-legacy-to-entries.js --apply
...
Behavior:
  - Reads all documents from the legacy collection and writes them to `entries`
  - Maps fields:
...
async function main() {
  ...
  console.log(`Found ${snapshot.size} documents in the legacy collection`);
  ...
  console.log('Migration complete. Note: original documents were not deleted.');
}
